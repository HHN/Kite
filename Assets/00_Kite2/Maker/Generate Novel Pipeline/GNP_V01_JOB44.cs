...
public class GNP_V01_JOB44 : PipelineJob
{
    public GNP_V01_JOB44()
    {
        this.jobName = "Job44 - Gesichtsausdruck Nachricht";
    }
    ...
    public override PipelineJobState HandleCompletion(string completion)
    {
        ...
        if (match.Success)
        {
            ...
            if (parseable)
            {
                //pipeline.GetMemory()[GenerateNovelPipeline.EXPRESSION_TYPE_33] = result;
    ...
    public override void InitializePrompt()
    {
        ...
        stringBuilder.Append("Hier die fragliche Nachricht: " + pipeline.GetMemory()[GenerateNovelPipeline.MESSAGE_33]);
        stringBuilder.Append("Die Nachricht wurde gesprochen von: " + pipeline.GetMemory()[GenerateNovelPipeline.SPEAKER_33]);
        stringBuilder.AppendLine();
