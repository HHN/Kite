...
public class GNP_V01_JOB01 : PipelineJob
{
    public GNP_V01_JOB01()
    {
        this.jobName = "Job01 - Generierung des Kontext für alle weiteren Prompts.";
    }
    ...
    public override PipelineJobState HandleCompletion(string completion)
    {
        ...
        if (match.Success)
        {
            ...
            pipeline.GetMemory()[GenerateNovelPipeline.CONTEXT] = result;
    ...
    public override void InitializePrompt()
    {
        ...
        stringBuilder.AppendLine();
        stringBuilder.Append("Deine Aufgabe ist es, einen gegebenen Text zu analysieren und daraus einen kontextbezogenen Teil für einen GPT-Prompt zu formulieren. Du sollst die Hauptthemen, Stimmungen und wichtigen Informationen aus dem Text extrahieren und diese in einer prägnanten und klaren Weise zusammenfassen. Diese Zusammenfassung dient als Kontextbasis für einen GPT-Prompt, der darauf aufbaut, um relevante und zielgerichtete Antworten zu generieren.");
        stringBuilder.AppendLine();

        // Kontext
        stringBuilder.Append("Der Kontext deiner Aufgabe:");
        stringBuilder.AppendLine();
        stringBuilder.Append("Der Text, den ich dir für diese Aufgabe zur Verfügung stelle, ist die Beschreibung einer Gründerin. In diesem Text beschreibt sie eine persönliche Geschichte, die sie erlebt hat, in der sie aufgrund ihres Geschlechts diskriminiert wurde.");
        stringBuilder.AppendLine();
        ...
        // Analyse Objekt
        stringBuilder.Append("Das Analyse Objekt deiner Aufgabe:");
        stringBuilder.AppendLine();
        stringBuilder.Append("Hier ist die Beschreibung der Geschichte der Gründerin: " + this.pipeline.GetMemory()[GenerateNovelPipeline.USER_ANSWER_01] + " " + this.pipeline.GetMemory()[GenerateNovelPipeline.USER_ANSWER_02] + " " + this.pipeline.GetMemory()[GenerateNovelPipeline.USER_ANSWER_03]);
        stringBuilder.AppendLine();
