...
public class Job03_ModifyAnswerThree : PipelineJob
{
    public Job03_ModifyAnswerThree()
    {
        this.jobName = "Modfiy Answer Three";
    }
    ...
    public override PipelineJobState HandleCompletion(string completion)
    {
        ...
        if (match.Success)
        {
            this.pipeline.GetMemory().Add(GenerateNovelPipeline.MODIFIED_ANSWER_03, match.Groups[1].Value);
    ...
    {
        string question = this.pipeline.GetMemory()[GenerateNovelPipeline.QUESTION_03];
        string answer = this.pipeline.GetMemory()[GenerateNovelPipeline.ANSWER_03];
        string templateAnswer = this.pipeline.GetMemory()[GenerateNovelPipeline.TEMPLATE_ANSWER_03];
