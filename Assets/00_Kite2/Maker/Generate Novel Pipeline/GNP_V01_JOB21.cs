...
public class GNP_V01_JOB21 : PipelineJob
{
    public GNP_V01_JOB21()
    {
        this.jobName = "Job21 - Gesichtsausdruck Nachricht";
    }
    ...
    public override PipelineJobState HandleCompletion(string completion)
    {
        ...
        if (match.Success)
        {
            ...
            if (parseable)
            {
                pipeline.GetMemory()[GenerateNovelPipeline.EXPRESSION_TYPE_07] = result;
    ...
    public override void InitializePrompt()
    {
        ...
        stringBuilder.Append("Hier die fragliche Nachricht: " + pipeline.GetMemory()[GenerateNovelPipeline.MESSAGE_07]);
        stringBuilder.Append("Die Nachricht wurde gesprochen von: " + pipeline.GetMemory()[GenerateNovelPipeline.SPEAKER_07]);
        stringBuilder.AppendLine();
