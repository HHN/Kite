...
public class GNP_V01_JOB26 : PipelineJob
{
    public GNP_V01_JOB26()
    {
        this.jobName = "Job26 - Gesichtsausdruck Nachricht";
    }
    ...
    public override PipelineJobState HandleCompletion(string completion)
    {
        ...
        if (match.Success)
        {
            ...
            if (parseable)
            {
                pipeline.GetMemory()[GenerateNovelPipeline.EXPRESSION_TYPE_13] = result;
    ...
    public override void InitializePrompt()
    {
        ...
        stringBuilder.Append("Hier die fragliche Nachricht: " + pipeline.GetMemory()[GenerateNovelPipeline.MESSAGE_13]);
        stringBuilder.Append("Die Nachricht wurde gesprochen von: " + pipeline.GetMemory()[GenerateNovelPipeline.SPEAKER_13]);
        stringBuilder.AppendLine();
