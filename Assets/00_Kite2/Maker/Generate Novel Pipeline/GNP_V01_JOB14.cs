...
public class GNP_V01_JOB14 : PipelineJob
{
    public GNP_V01_JOB14()
    {
        this.jobName = "Job14 - Erzeugung von alternativen Antworten an der dritten Frage Stelle.";
    }
    ...
    public bool HandleReturnValue(string jsonString)
    {
        try
        {
            ...
            foreach (var item in items)
            {
                ...
                if (id == 1)
                {
                    this.pipeline.GetMemory()[GenerateNovelPipeline.MESSAGE_10_OPTION_02] = value;
                }
                else if (id == 2)
                {
                    this.pipeline.GetMemory()[GenerateNovelPipeline.MESSAGE_10_OPTION_03] = value;
                }
                else if (id == 3)
                {
                    this.pipeline.GetMemory()[GenerateNovelPipeline.MESSAGE_10_OPTION_04] = value;
                }
                else
                {
