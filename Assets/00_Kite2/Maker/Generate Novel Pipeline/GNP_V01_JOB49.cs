...
public class GNP_V01_JOB49 : PipelineJob
{
    public GNP_V01_JOB49()
    {
        this.jobName = "Job49 - Gesichtsausdruck Nachricht";
    }
    ...
    public override PipelineJobState HandleCompletion(string completion)
    {
        ...
        if (match.Success)
        {
            ...
            if (parseable)
            {
                pipeline.GetMemory()[GenerateNovelPipeline.EXPRESSION_TYPE_38] = result;
    ...
    public override void InitializePrompt()
    {
        ...
        stringBuilder.Append("Hier die fragliche Nachricht: " + pipeline.GetMemory()[GenerateNovelPipeline.MESSAGE_38]);
        stringBuilder.Append("Die Nachricht wurde gesprochen von: " + pipeline.GetMemory()[GenerateNovelPipeline.SPEAKER_38]);
        stringBuilder.AppendLine();
