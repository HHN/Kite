...
public class GNP_V01_JOB45 : PipelineJob
{
    public GNP_V01_JOB45()
    {
        this.jobName = "Job45 - Gesichtsausdruck Nachricht";
    }
    ...
    public override PipelineJobState HandleCompletion(string completion)
    {
        ...
        if (match.Success)
        {
            ...
            if (parseable)
            {
                //pipeline.GetMemory()[GenerateNovelPipeline.EXPRESSION_TYPE_34] = result;
    ...
    public override void InitializePrompt()
    {
        ...
        stringBuilder.Append("Hier die fragliche Nachricht: " + pipeline.GetMemory()[GenerateNovelPipeline.MESSAGE_34]);
        stringBuilder.Append("Die Nachricht wurde gesprochen von: " + pipeline.GetMemory()[GenerateNovelPipeline.SPEAKER_34]);
        stringBuilder.AppendLine();
