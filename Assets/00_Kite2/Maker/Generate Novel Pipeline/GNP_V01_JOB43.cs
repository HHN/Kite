...
public class GNP_V01_JOB43 : PipelineJob
{
    public GNP_V01_JOB43()
    {
        this.jobName = "Job43 - Gesichtsausdruck Nachricht";
    }
    ...
    public override PipelineJobState HandleCompletion(string completion)
    {
        ...
        if (match.Success)
        {
            ...
            if (parseable)
            {
                //pipeline.GetMemory()[GenerateNovelPipeline.EXPRESSION_TYPE_32] = result;
    ...
    public override void InitializePrompt()
    {
        ...
        stringBuilder.Append("Hier die fragliche Nachricht: " + pipeline.GetMemory()[GenerateNovelPipeline.MESSAGE_32]);
        stringBuilder.Append("Die Nachricht wurde gesprochen von: " + pipeline.GetMemory()[GenerateNovelPipeline.SPEAKER_32]);
        stringBuilder.AppendLine();
