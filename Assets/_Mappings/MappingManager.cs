...
namespace Assets._Scripts.Player.KiteNovels.VisualNovelFormatter
{
    public class MappingManager : MonoBehaviour
    {
        ...
        // Mapping-Files
        private static readonly string MappingFileBias = Path.Combine(Application.dataPath, "_Mappings/BiasMapping.txt");
        private static readonly string MappingFileFaceExpression = Path.Combine(Application.dataPath, "_Mappings/FaceExpressionMapping.txt");

        // Dictionaries
        private static Dictionary<string, string> biasMapping = LoadBiasMapping();
        private static Dictionary<string, int> faceExpressionMapping = LoadFaceExpressionMapping();
        ...
        /// </summary>
        /// <returns>Ein Dictionary, das den FaceExpression-String (Key) auf den zugehörigen Integer-Wert (Value) abbildet.</returns>
        private static Dictionary<string, int> LoadFaceExpressionMapping()
        {
            ...
            try
            {
                // Prüfe, ob die Mapping-Datei existiert.
                if (!File.Exists(MappingFileFaceExpression))
                {
                    ...
                    return mapping; // Gib ein leeres Mapping zurück.
                }
                ...
                foreach (string line in lines)
                {
                    ...
                    if (colonIndex > 0 && colonIndex < line.Length - 1)
                    {
                        // Extrahiere den FaceExpression-String (Key) und den dazugehörigen Wert als String.
                        string key = line.Substring(0, colonIndex).Trim();
                        ...
                        {
                            // Füge den Key und den geparsten Wert dem Dictionary hinzu, sofern der Key nicht leer ist
        ...
        /// <summary>
        /// Übersetzt den übergebenen FaceExpression-String in seinen zugehörigen Integer-Wert anhand des geladenen Mappings.
        /// Falls kein Mapping gefunden wird, wird -1 zurückgegeben.
        /// </summary>
        ...
        {
            if (faceExpressionMapping.TryGetValue(faceExpression, out int id))
            {
                return id;
            }
            else
