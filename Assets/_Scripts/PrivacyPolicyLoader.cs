...
public class PrivacyPolicyLoader : MonoBehaviour
{
    ...
    {
        // Läd die Textdatei aus dem Resources-Ordner
        ...
        foreach (string rawBlock in rawBlocks)
        {
            ...
            {
                // Normale Textblöcke: Links erkennen und formatieren
                string processedText = linkRegex.Replace(block, (Match m) =>
                {
                    string url = m.Value;
                    if (!url.StartsWith("http"))
                        url = "http://" + url;
                    // Unterstreiche den Linktext zusätzlich mittels <u> Tags und verwende Rich-Text für Farbe
                ...
                // Füge einen Link-Handler hinzu, der Klicks auf Links verarbeitet
                blockGO.AddComponent<TMP_LinkHandler>();
            }
        }
    }
}
