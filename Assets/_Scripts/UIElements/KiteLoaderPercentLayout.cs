...
namespace Assets._Scripts.UIElements
{
    ...
    public class KiteLoaderPercentLayout : MonoBehaviour
    {
        ...
        private void Apply()
        {
            ...
            // --- Structure / Parenting ---
            if (panel && circle.parent != panel.parent) circle.SetParent(panel.parent, false); // Set Circle under the same parent as the Panel
            if (parentInnerLogoToCircle && innerLogo && innerLogo.parent != circle) innerLogo.SetParent(circle, false);
            ...
            if (cImg) cImg.raycastTarget = false;
            var iImg = innerLogo ? innerLogo.GetComponent<Image>() : null;
            if (iImg) iImg.raycastTarget = false;
            ...
            {
                float wmW = Mathf.Clamp(wordmarkPercentOfContainer * (Wpx / sf), wordmarkMinPx / sf, wordmarkMaxPx / sf);
                float wmH = wmW * 0.25f;
